...
function PageBanner({ text, backgroundImage, links }) {
    return (
        <div className='relative bg-cover bg-center flex flex-col items-center justify-center py-40 sm:py-40 md:py-40' style={{ backgroundImage: `url(${backgroundImage})` }}>
            ...
            <div className='flex flex-row gap-2 sm:gap-8 text-white z-10 text text-md md:text-lg lg:text-xl'>
                {links?.map((link, index) => (
                    <React.Fragment key={index}>
                        <a href={link.link} className='hover:scale-110 font-thin text-center w-full sm:w-auto transition-all'>
                            <h2>{link.name}</h2>
                        </a>
                    </React.Fragment>
                ))}
